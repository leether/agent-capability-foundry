...
This repository is inspired by prior public agent-skill ecosystems, but it
restructures that direction into a governed, agent-native, capability-oriented
model.

The current self-evolution direction is specifically adapted from the
`self-improving-agent` lineage in `agent-playbook`, with substantial changes in
packaging, governance, runtime structure, and documentation policy.

See [ATTRIBUTION.md](./ATTRIBUTION.md) for source details and adaptation scope.
